...
export function Receipts() {
  ...
  const handleApprove = async (receiptId: string) => {
    if (window.confirm('Aprovar este comprovante? O pagamento será marcado como pago.')) {
      await approveReceipt(receiptId);
    }
  };

  const handleReject = async (receiptId: string) => {
    if (window.confirm('Rejeitar este comprovante?')) {
    ...
    <div className="flex-1 flex flex-col h-full space-y-4 min-h-0">
      ...
      <div className="flex-1 bg-white border border-slate-200 rounded-lg shadow-sm flex flex-col min-h-0 overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between shrink-0">
          <h3 className="text-sm font-bold text-slate-600">Comprovantes de Pagamento</h3>
        </div>
        
        <div className="flex-1 overflow-y-auto overflow-x-auto">
          {filteredReceipts.length === 0 ? (
            <div className="text-center py-12 text-slate-500 text-sm font-medium">
              <p>Nenhum comprovante {filter !== 'Todos' ? `com status "${filter}"` : 'encontrado'}.</p>
            </div>
          ) : (
                ...
                <div key={receipt.id} className="p-4 hover:bg-slate-50 transition-colors">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <p className="text-sm font-bold text-slate-800 mb-1">{receipt.description}</p>
                      <div className="flex flex-wrap items-center gap-3 text-[10px] text-slate-500">
                        <span className="flex items-center gap-1">
                          <User size={10} />
                          {getMemberName(receipt.memberId)}
                        </span>
                        <span className="flex items-center gap-1">
                          <Calendar size={10} />
                          Mês: {getMonthName(receipt.paymentId ? getPaymentMonth(receipt.paymentId) : '')}
                        </span>
                      ...
                      {receipt.reviewedBy && (
                        <p className="text-[10px] text-slate-400 mt-1">
                          Revisado por {receipt.reviewedBy} em {receipt.reviewedAt?.split('T')[0].split('-').reverse().join('/')}
                        </p>
                      )}
                    </div>
